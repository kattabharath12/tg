...
export class AzureDocumentIntelligenceService {
  ...
  private extract1099IntFieldsFromOCR(ocrText: string, baseData: ExtractedFieldData): ExtractedFieldData {
    ...
    // ENHANCED: Complete 1099-INT OCR extraction patterns for all 15 boxes
    const amountPatterns = {
      // Box 1: Interest income
      interestIncome: [
        /1\s+Interest\s+income\s*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /Box\s*1[^\d]*Interest\s+income[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /(?:^|\n)\s*1\s+[^\d]*\$?([0-9,]+\.?\d{0,2})/m
      ],
      ...
      earlyWithdrawalPenalty: [
        ...
        /Box\s*2[^\d]*Early\s+withdrawal\s+penalty[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /(?:^|\n)\s*2\s+[^\d]*\$?([0-9,]+\.?\d{0,2})/m
      ],
      
      // Box 3: Interest on U.S. Savings Bonds and Treasury obligations
      interestOnUSavingsBonds: [
        /3\s+Interest\s+on\s+U\.?S\.?\s+Savings\s+Bonds\s+and\s+Treasury\s+obligations\s*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /3\s+Interest\s+on\s+U\.?S\.?\s+Treasury\s+obligations\s*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /Box\s*3[^\d]*Interest\s+on\s+U\.?S\.?\s+[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /(?:^|\n)\s*3\s+[^\d]*\$?([0-9,]+\.?\d{0,2})/m
      ],
      ...
      federalTaxWithheld: [
        ...
        /Box\s*4[^\d]*Federal\s+income\s+tax\s+withheld[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /(?:^|\n)\s*4\s+[^\d]*\$?([0-9,]+\.?\d{0,2})/m
      ],
      ...
      investmentExpenses: [
        ...
        /Box\s*5[^\d]*Investment\s+expenses[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /(?:^|\n)\s*5\s+[^\d]*\$?([0-9,]+\.?\d{0,2})/m
      ],
      ...
      foreignTaxPaid: [
        ...
        /Box\s*6[^\d]*Foreign\s+tax\s+paid[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /(?:^|\n)\s*6\s+[^\d]*\$?([0-9,]+\.?\d{0,2})/m
      ],
      ...
      taxExemptInterest: [
        ...
        /Box\s*8[^\d]*Tax-exempt\s+interest[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /(?:^|\n)\s*8\s+[^\d]*\$?([0-9,]+\.?\d{0,2})/m
      ],
      ...
      specifiedPrivateActivityBondInterest: [
        ...
        /Box\s*9[^\d]*Specified\s+private\s+activity\s+bond\s+interest[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /9\s+Private\s+activity\s+bond\s+interest\s*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /(?:^|\n)\s*9\s+[^\d]*\$?([0-9,]+\.?\d{0,2})/m
      ],
      ...
      marketDiscount: [
        ...
        /Box\s*10[^\d]*Market\s+discount[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
      ...
      bondPremium: [
        ...
        /Box\s*11[^\d]*Bond\s+premium[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /(?:^|\n)\s*11\s+[^\d]*\$?([0-9,]+\.?\d{0,2})/m
      ],
      
      // Box 13: State tax withheld
      stateTaxWithheld: [
        /13\s+State\s+tax\s+withheld\s*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /Box\s*13[^\d]*State\s+tax\s+withheld[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /(?:^|\n)\s*13\s+[^\d]*\$?([0-9,]+\.?\d{0,2})/m
      ],
      
      // Box 15: State interest
      stateInterest: [
        /15\s+State\s+interest\s*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /Box\s*15[^\d]*State\s+interest[^\d]*:?\s*\$?([0-9,]+\.?\d{0,2})/i,
        /(?:^|\n)\s*15\s+[^\d]*\$?([0-9,]+\.?\d{0,2})/m
      ]
    };
    ...
    // ENHANCED: Extract text fields (Box 7 and Box 14)
    const textPatterns = {
      // Box 7: Foreign country or U.S. possession
      foreignCountry: [
        /7\s+Foreign\s+country\s+or\s+U\.?S\.?\s+possession\s*:?\s*([A-Za-z\s]+?)(?:\n|$|Box|8)/i,
        /Box\s*7[^\n]*Foreign\s+country[^\n]*:?\s*([A-Za-z\s]+?)(?:\n|$|Box)/i,
        /(?:^|\n)\s*7\s+[^:\n]*:?\s*([A-Za-z\s]{2,30})(?:\n|$|Box)/m
      ],
      
      // Box 14: State/Payer's state no.
      statePayerNumber: [
        /14\s+State\/Payer's\s+state\s+no\.\s*:?\s*([A-Za-z0-9\-]+?)(?:\n|$|Box|15)/i,
        /Box\s*14[^\n]*State[^\n]*:?\s*([A-Za-z0-9\-]+?)(?:\n|$|Box)/i,
        /(?:^|\n)\s*14\s+[^:\n]*:?\s*([A-Za-z0-9\-]{1,20})(?:\n|$|Box)/m
      ]
    };
    ...
    // Extract text fields
    for (const [fieldName, patterns] of Object.entries(textPatterns)) {
      for (const pattern of patterns) {
        const match = ocrText.match(pattern);
        if (match && match[1]) {
          const textValue = match[1].trim();
          
          if (textValue.length > 0 && textValue.length < 50) {
            data[fieldName] = textValue;
            console.log(`✅ [Azure DI OCR] Found ${fieldName}: ${textValue}`);
            break;
          }
        }
      }
